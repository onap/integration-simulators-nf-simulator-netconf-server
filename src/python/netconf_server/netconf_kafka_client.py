...
logging.basicConfig(filename='netconf_kafka_client.log', level=logging.DEBUG)
...
class NetconfKafkaClient(object):
    ...
    def get_all_messages_from(self, topic: str) -> list:
        logging.debug("Getting config changes from topic %s" % topic)

        messages = []
        consumer = self._get_kafka_consumer(topic)
        for message in consumer:
            message_value = message.value
            logging.debug("Fetched config change %s" % message_value)
